...
namespace Repository
{
    public class CompanyReqRepository : RepositoryBase<CompanyReq>, ICompanyReqRepository
    {
        ...
        public IEnumerable<CompanyReq> GetAllSuggestedWorkers(Guid companyId, bool trackChanges) =>FindAll(trackChanges).OrderBy(x => x.CompanyId).ToList(); public IEnumerable<CompanyReq> GetAllSuggestedCompany(Guid workerId, bool trackChanges) => FindAll(trackChanges).OrderBy(x => x.WorkerId).ToList();
